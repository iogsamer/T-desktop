...
bool StartNodeServer()
{
    ...
    if (!FileExists(exePath) || !FileExists(scriptPath)) {
        ...
        if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, localAppData))) {
            // Check DarkFlix service path first
            std::wstring darkflixDir = std::wstring(localAppData) + L"\\Programs\\DarkFlix";
            std::wstring darkflixExe = darkflixDir + L"\\stremio-runtime.exe";
            std::wstring darkflixScript = darkflixDir + L"\\server.js";

            // Fallback: legacy StremioService path
            ...
            if (FileExists(darkflixExe) && FileExists(darkflixScript)) {
                exePath = darkflixExe;
                scriptPath = darkflixScript;
                exeDir = darkflixDir;
            } else if (FileExists(altExePath) && FileExists(altScriptPath)) {
                exePath = altExePath;
...
// Launches: node.exe packages/darkflix-auth/src/server.js
// on port 11471 (localhost only).

bool StartAuthServer()
{
    std::wstring exeDir = GetExeDirectory();

    // Look for node.exe shipped with the app, or system node
    std::wstring nodeExe = exeDir + L"\\node.exe";
    if (!FileExists(nodeExe)) nodeExe = L"node.exe";   // rely on PATH

    std::wstring authScript = exeDir + L"\\packages\\darkflix-auth\\src\\server.js";
    if (!FileExists(authScript)) {
        std::cout << "[auth] WARNING: darkflix-auth server.js not found at: "
                  << std::string(authScript.begin(), authScript.end()) << "\n";
        return false;
    }
    ...
    STARTUPINFOW si; ZeroMemory(&si,sizeof(si));
    si.cb        = sizeof(si);
    si.hStdOutput= outW; si.hStdError=outW;
    si.dwFlags   = STARTF_USESTDHANDLES;

    PROCESS_INFORMATION pi; ZeroMemory(&pi,sizeof(pi));
    std::wstring cmdLine = L"\"" + nodeExe + L"\" \"" + authScript + L"\"";

    SetEnvironmentVariableW(L"DARKFLIX_AUTH_PORT", std::to_wstring(DARKFLIX_AUTH_PORT).c_str());
    ...
    if (!ok) {
        CloseHandle(outR);
        std::cout << "[auth] Failed to start auth server. GetLastError=" << GetLastError() << "\n";
        return false;
    }

    AssignProcessToJobObject(g_serverJob, pi.hProcess);
    g_authProcess = pi.hProcess;
    g_authRunning = true;
    CloseHandle(pi.hThread);
    CloseHandle(outR);   // detach; we don't read auth output in a thread

    std::cout << "[auth] DarkFlix auth server started (pid=" << pi.dwProcessId << ").\n";
    return true;
}
...
void StopAuthServer()
{
    if (g_authRunning) {
        g_authRunning = false;
        if (g_authProcess) {
            TerminateProcess(g_authProcess, 0);
            WaitForSingleObject(g_authProcess, INFINITE);
            CloseHandle(g_authProcess); g_authProcess = nullptr;
        }
        std::cout << "[auth] Auth server stopped.\n";
    }
}
