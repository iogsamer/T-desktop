...
int main(int argc, char *argv[]) {
  ...
  // check for duplicate runtime processes
  std::vector<std::wstring> processesToCheck = {L"darkflix.exe",
                                                L"stremio-runtime.exe"};
  if (IsDuplicateProcessRunning(processesToCheck)) {
    MessageBoxW(nullptr,
                L"An older version of DarkFlix or DarkFlix server may be "
                L"running. There could be issues.",
                L"DarkFlix Already Running", MB_OK | MB_ICONWARNING);
  }
  ...
  g_darkBrush = CreateSolidBrush(RGB(18, 8, 2));
  ...
  // DarkFlix auth server (always starts — needed for login gate)
  StartAuthServer();
