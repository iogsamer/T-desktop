...
{
    g_hMutex = CreateMutexW(nullptr, FALSE, L"SingleInstanceMtx_DarkFlixWebShell");
    if(!g_hMutex){
    ...
    for(int i=1; i<argc; ++i){
        ...
        if(size_needed > 0){
            ...
            if(argW.rfind(L"darkflix://",0)==0 || argW.rfind(L"magnet:",0)==0 || FileExists(argW)) {
                protocolArg = argW;
                break;
            }
        }
...
void HandleInboundJSON(const std::string &msg)
{
    try {
        ...
        // ── DarkFlix Auth Complete ────────────────────────────────
        if (j.contains("type") && j["type"].is_string() &&
            j["type"].get<std::string>() == "DARKFLIX_AUTH_COMPLETE")
        {
            std::cout << "[AUTH]: DARKFLIX_AUTH_COMPLETE received." << std::endl;
            // Profile embedded in JWT — no manifest needed
            PostMessage(g_hWnd, WM_AUTH_DONE, 0, 0);
            return;
        }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_REACHABILITY_DONE: {
        // AuthResult* packed by background thread in webview.cpp
        // Contains both the Stremio URL and auth status — no HTTP on UI thread.
        struct AuthResult { std::wstring url; bool auth; };
        auto* result = reinterpret_cast<AuthResult*>(wParam);
        if (result) {
            if (!result->url.empty() && g_webview) {
                g_webuiUrl = result->url;
                if (result->auth) {
                    std::wcout << L"[AUTH]: Authenticated — loading Stremio." << std::endl;
                    g_webview->Navigate(result->url.c_str());
                } else {
                    std::wcout << L"[AUTH]: Not authenticated — showing login portal." << std::endl;
                    g_webview->Navigate(L"http://127.0.0.1:11471/");
                }
            } else {
                MessageBoxW(nullptr,
                    L"All streaming endpoints are unreachable.\nCheck your internet connection.",
                    L"DarkFlix — Connection Error", MB_ICONERROR | MB_OK);
            }
            delete result;
        }
        break;
        ...
        case WM_AUTH_DONE: {
        // Auth portal sent DARKFLIX_AUTH_COMPLETE — navigate to Stremio
        std::wcout << L"[AUTH]: Authentication complete. Navigating to Stremio." << std::endl;
        if (g_webview && !g_webuiUrl.empty()) {
            // Addons installed via AddScriptToExecuteOnDocumentCreated
            g_webview->Navigate(g_webuiUrl.c_str());
        }
        break;
    }
    ...
    case WM_COPYDATA:
    {
        ...
        if (pcds && pcds->dwData == 1 && pcds->lpData) {
            ...
            if (FileExists(receivedUrl)) {
            ...
            } else if (receivedUrl.rfind(L"darkflix://detail", 0) == 0) {
                std::string utf8Url = WStringToUtf8(receivedUrl);
                json j;
                j["type"] = "ReplaceLocation";
                j["path"] = utf8Url;
                SendToJS("ReplaceLocation", j);
            } else if (receivedUrl.rfind(L"darkflix://", 0) == 0) {
                // Handle darkflix:// protocol
                std::string utf8Url = WStringToUtf8(receivedUrl);
                json j;
                j["type"] = "AddonInstall";
                j["path"] = utf8Url;
                ...
                std::wcout << L"Received URL is neither a valid file nor a darkflix:// protocol." << std::endl;
            }
        }
        return 0;
    }
