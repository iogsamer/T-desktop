...
(function() {
  ...
  btn.style.backgroundColor = '#1a0800'; // DarkFlix Orange-Dark
  ...
  btn.style.boxShadow = '0 4px 12px rgba(255,100,0,0.35)';
  btn.style.cursor = 'pointer';
  btn.style.transition = 'background-color 0.3s ease';

  // Hover effect:
  btn.addEventListener('mouseenter', function() {
    btn.style.backgroundColor = '#cc5500'; // DarkFlix Orange Hover
  });
  btn.addEventListener('mouseleave', function() {
    btn.style.backgroundColor = '#1a0800';
  });

  // Create an image element for the logo
  var img = document.createElement('img');
  img.src = 'data:image/png;base64,';
  img.alt = 'Logo';
...
img.addEventListener('error', function() {
  ...
  txt.textContent = 'Back to DarkFlix';
...
// Runs on every document creation — installs DarkFlix addons silently.
// Skips non-Stremio pages (auth portal, Google OAuth, etc.)
static const wchar_t* DARKFLIX_ADDON_SCRIPT = LR"ADDON(
(function() {
  var h = location.hostname;
  var isStremio = h === 'stremio.zarg.me' || h === 'web.stremio.com' || h === 'zaarrg.github.io';
  if (!isStremio) return;
  var ADDONS = [
    'https://opensubtitlesv3-pro.dexter21767.com/eyJsYW5ncyI6WyJhcmFiaWMiXSwic291cmNlIjoiYWxsIiwiYWlUcmFuc2xhdGVkIjpmYWxzZSwiYXV0b0FkanVzdG1lbnQiOnRydWV9/manifest.json',
    'https://torrentio.strem.fun/sort=qualitysize|language=arabic/manifest.json'
  ];
  var done = false;
  var tries = 0;
  var t = setInterval(function() {
    if (done || ++tries > 60) { clearInterval(t); return; }
    try {
      var tr = window.transport;
      if (tr && typeof tr.dispatch === 'function') {
        ADDONS.forEach(function(u) {
          tr.dispatch({ action: 'Ctx', args: { action: 'InstallAddon', args: { transportUrl: u } } });
        });
        done = true; clearInterval(t);
        console.log('[DarkFlix] Addons installed');
      }
    } catch(e) {}
  }, 1500);
})();
            ...
            env->CreateCoreWebView2Controller(
                ...
                [hWnd](HRESULT result, ICoreWebView2Controller* rawController)->HRESULT
                {
                    ...
                    if(settings) {
                        ...
                        std::wstring customUA = std::wstring(L"DarkFlixShell/") + Utf8ToWstring(APP_VERSION);
                    ...
                    g_webview->AddScriptToExecuteOnDocumentCreated(DARKFLIX_ADDON_SCRIPT,nullptr);
                    ...
                    std::thread([](){
                        ...
                        // Wait for auth server to be ready (up to 8 s) — avoids UI-thread HTTP
                        bool authReady = false;
                        for (int attempt = 0; attempt < 16 && !authReady; ++attempt) {
                            HINTERNET hS = WinHttpOpen(L"DarkFlix/1.0",
                                WINHTTP_ACCESS_TYPE_NO_PROXY,WINHTTP_NO_PROXY_NAME,WINHTTP_NO_PROXY_BYPASS,0);
                            if (hS) {
                                HINTERNET hC = WinHttpConnect(hS,L"127.0.0.1",DARKFLIX_AUTH_PORT,0);
                                if (hC) {
                                    HINTERNET hR = WinHttpOpenRequest(hC,L"GET",L"/api/health",nullptr,
                            ...
                            if (!authReady) {
                                std::wcout << L"[AUTH]: Server not ready, retrying (" << attempt+1 << L"/16)..." << std::endl;
                                std::this_thread::sleep_for(std::chrono::milliseconds(500));
                            }
                        }

                        // Check authentication status (still in background thread)
                        ...
                        if (authReady) {
                            ...
                            if (hS) {
                                HINTERNET hC = WinHttpConnect(hS,L"127.0.0.1",DARKFLIX_AUTH_PORT,0);
                                if (hC) {
                                    HINTERNET hR = WinHttpOpenRequest(hC,L"GET",L"/api/auth/status",nullptr,
                                        WINHTTP_NO_REFERER,WINHTTP_DEFAULT_ACCEPT_TYPES,WINHTTP_FLAG_BYPASS_PROXY_CACHE);
                                    if (hR) {
                                        // Forward session cookie from Windows cookie store (WebView2 shares it)
                                        if (WinHttpSendRequest(hR,WINHTTP_NO_ADDITIONAL_HEADERS,0,nullptr,0,0,0) &&
                                            WinHttpReceiveResponse(hR,nullptr)) {
                                            DWORD st=0,sl=sizeof(st);
                                            WinHttpQueryHeaders(hR,WINHTTP_QUERY_STATUS_CODE|WINHTTP_QUERY_FLAG_NUMBER,
                                            ...
                                            if (st==200) {
                                                ...
                                                while (WinHttpQueryDataAvailable(hR,&avail) && avail>0
                                                       && body.size()<MAX_BODY) {
                                                    DWORD safe=min(avail,(DWORD)(MAX_BODY-body.size()));
                                                    std::vector<char> buf(safe+1,0); DWORD rd=0;
                                                    WinHttpReadData(hR,buf.data(),safe,&rd);
                                                    body.append(buf.data(),rd);
                                                }
                                                isAuthenticated=(body.find("\"authenticated\":true")!=std::string::npos);
                                            }
                                        }
                                        WinHttpCloseHandle(hR);
                                    }
                        ...
                        std::wcout << L"[AUTH]: Status=" << (isAuthenticated?L"authenticated":L"not authenticated") << std::endl;

                        // Pack both results and post to UI thread
                        struct AuthResult { std::wstring url; bool auth; };
                        auto* pResult = new AuthResult{ foundUrl, isAuthenticated };
...
static void SetupWebMessageHandler()
{
    ...
    g_webview->add_NavigationCompleted(
        ...
        [](ICoreWebView2* sender, ICoreWebView2NavigationCompletedEventArgs* args)->HRESULT
        {
            ...
            // Add back to DarkFlix button if not on DarkFlix
