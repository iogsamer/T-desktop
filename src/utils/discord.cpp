...
static void SetDiscordWatchingPresence(
    ...
    // 12: watch on DarkFlix button link (darkflix link) (Optional)
    ...
    if (!args[12].empty()) {
        discordPresence.button2Label = "Watch on DarkFlix";
...
static void SetDiscordDiscoverPresence(const char *const details, const char *const state) {
    ...
    discordPresence.largeImageKey = "darkflix_logo";
    discordPresence.largeImageText = "DarkFlix";
    Discord_UpdatePresence(&discordPresence);
}
