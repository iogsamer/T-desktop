...
bool URLContainsAny(const std::wstring& url) {
    ...
    // Always allow the local auth portal and Google OAuth flow
    static const std::vector<std::wstring> alwaysAllow = {
        L"127.0.0.1:11471",
        L"accounts.google.com",
        L"oauth2.googleapis.com",
        L"googleapis.com/oauth2",
    };
    for (const auto& a : alwaysAllow) {
        if (url.find(a) != std::wstring::npos) return true;
    }
