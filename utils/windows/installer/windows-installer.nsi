...
!define APP_NAME "DarkFlix"
!define PRODUCT_VERSION "$%package_version%"
!define ARCH "$%arch%"
!searchparse "${PRODUCT_VERSION}" `` VERSION_MAJOR `.` VERSION_MINOR `.` VERSION_REVISION
!define APP_URL "https://github.com/YourOrg/darkflix-desktop"
!define DATA_FOLDER "darkflix"

!define COMPANY_NAME "DarkFlix Team"
...
InstallDir "$LOCALAPPDATA\Programs\DarkFlix\${APP_NAME}-${VERSION_MAJOR}"
InstallDirRegKey HKLM Software\DarkFlix\DarkFlix InstallLocation
...
!define APP_LAUNCHER "darkflix.exe"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\darkflix.exe"
...
XPStyle off
!define MUI_INSTALLCOLORS "120802 FF7800"
...
LangString noRoot ${LANG_ENGLISH} "You cannot install DarkFlix in a directory that requires administrator permissions"
...
Var checkbox_value
Var InstallDarkFlixServiceCheckbox

Function fin_pg_options
    ; Install DarkFlix Service checkbox (top)
    ${NSD_CreateCheckbox} 180 -100 100% 8u "Install DarkFlix Service"
    Pop $InstallDarkFlixServiceCheckbox
    SetCtlColors $InstallDarkFlixServiceCheckbox '0xFF0000' '0xFFFFFF'
    ${NSD_Uncheck} $InstallDarkFlixServiceCheckbox
...
Function fin_pg_leave
    ; Check "Install DarkFlix Service" checkbox state
    ${NSD_GetState} $InstallDarkFlixServiceCheckbox $checkbox_value
    ${If} $checkbox_value == ${BST_CHECKED}
        SetDetailsPrint textonly
        DetailPrint "Installing DarkFlix Service..."
        ...
        SetDetailsPrint textonly
        ${If} $R0 == 0
            DetailPrint "DarkFlix Service installed successfully."
        ${Else}
            DetailPrint "Failed to install DarkFlix Service (error: $R0)."
            MessageBox MB_OK|MB_ICONEXCLAMATION "Error installing DarkFlix Service (error code: $R0)."
        ${EndIf}
        SetDetailsPrint none
    ${EndIf}
    ...
    ${If} $checkbox_value == ${BST_CHECKED}
        !insertmacro APP_ASSOCIATE "torrent" "darkflix" "BitTorrent file" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
    ${EndIf}

    ; Set friendly name for DarkFlix in "Open With" menu
    WriteRegStr HKCU "Software\Classes\darkflix" "FriendlyTypeName" "${APP_NAME}"
    WriteRegStr HKCU "Software\Classes\darkflix\shell\open" "FriendlyAppName" "${APP_NAME}"
    ...
    ${If} $checkbox_value == ${BST_CHECKED}
        ...
        WriteRegStr HKCU "Software\Classes\magnet\DefaultIcon" "" "$INSTDIR\darkflix.exe,0"
        WriteRegStr HKCU "Software\Classes\magnet\shell\open\command" "" '"$INSTDIR\darkflix.exe" "%1"'
    ${EndIf}
    ...
    !macro APP_ASSOCIATE_EXTENSIONS
        ...
        LoopStart:
            ...
            ${If} $CurrentExtension != ""
                ...
                !insertmacro APP_ASSOCIATE "$CurrentExtension" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
                ; Continue the loop
                Goto LoopStart
            ${EndIf}
        ; End of the loop
    ...
    !macroend
    ${If} $checkbox_value == ${BST_CHECKED}
        !insertmacro APP_ASSOCIATE "mp4" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "mkv" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "avi" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "mov" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "wmv" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "flv" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "webm" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "mpg" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "mpeg" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "3gp" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "m4v" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "ts" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "vob" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "f4v" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "m2ts" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "asf" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "divx" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "ogv" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "rm" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
        !insertmacro APP_ASSOCIATE "rmvb" "darkflix" "Media File" "$INSTDIR\darkflix.exe,0" "Play with DarkFlix" "$INSTDIR\darkflix.exe $\"%1$\""
    ${EndIf}
FunctionEnd
...
Function ${un}RemoveAllExceptDefaultProfile
  ...
  StrCpy $R1 "$INSTDIR\darkflix.exe.WebView2\EBWebView"            ; Root directory to operate on
...
;  "darkflix.exe.WebView2\darkflix.exe.WebView2\EBWebView" and "portable_config" folder.
; ---------------------------------------------------
!macro RemoveAllExceptWebView2 un
Function ${un}RemoveAllExceptWebView2
  ; Hardcoded values for your scenario
  StrCpy $R0 "darkflix.exe.WebView2" ; Directory to exclude
...
Function .onInit ; check for previous version
    ...
    StrCpy $R1 "$LOCALAPPDATA\Programs\DarkFlix\DarkFlix-5"
...
Section ; Shortcuts
    ...
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\${APP_NAME}.lnk" "$INSTDIR\darkflix.exe" "" "$INSTDIR\darkflix.exe" "" "" "" "${APP_NAME} ${PRODUCT_VERSION}"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Uninstall ${APP_NAME}.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\darkflix.exe" "" "" "" "Uninstall ${APP_NAME}"
    ...
    WriteRegStr HKCU "${UNINSTALL_KEY}" "DisplayIcon" "$INSTDIR\darkflix.exe"
    ...
    WriteRegStr HKCU "Software\Classes\darkflix" "" "URL:DarkFlix Protocol"
    WriteRegStr HKCU "Software\Classes\darkflix" "URL Protocol" ""
    WriteRegStr HKCU "Software\Classes\darkflix\DefaultIcon" "" "$INSTDIR\darkflix.exe,1"
    WriteRegStr HKCU "Software\Classes\darkflix\shell" "" "open"
    WriteRegStr HKCU "Software\Classes\darkflix\shell\open\command" "" '"$INSTDIR\darkflix.exe" "%1"'
...
Section "uninstall"
    ...
    DeleteRegKey HKCU Software\Classes\darkflix
    DeleteRegKey HKCU Software\Classes\magnet

    ; Remove friendly name registry entry
    DeleteRegKey HKCR Applications\darkflix.exe

    !insertmacro APP_UNASSOCIATE "torrent" "darkflix"
    !insertmacro APP_UNASSOCIATE "mp4" "darkflix"
    !insertmacro APP_UNASSOCIATE "mkv" "darkflix"
    !insertmacro APP_UNASSOCIATE "avi" "darkflix"
    !insertmacro APP_UNASSOCIATE "mov" "darkflix"
    !insertmacro APP_UNASSOCIATE "wmv" "darkflix"
    !insertmacro APP_UNASSOCIATE "flv" "darkflix"
    !insertmacro APP_UNASSOCIATE "webm" "darkflix"
    !insertmacro APP_UNASSOCIATE "mpg" "darkflix"
    !insertmacro APP_UNASSOCIATE "mpeg" "darkflix"
    !insertmacro APP_UNASSOCIATE "3gp" "darkflix"
    !insertmacro APP_UNASSOCIATE "m4v" "darkflix"
    !insertmacro APP_UNASSOCIATE "ts" "darkflix"
    !insertmacro APP_UNASSOCIATE "vob" "darkflix"
    !insertmacro APP_UNASSOCIATE "f4v" "darkflix"
    !insertmacro APP_UNASSOCIATE "m2ts" "darkflix"
    !insertmacro APP_UNASSOCIATE "asf" "darkflix"
    !insertmacro APP_UNASSOCIATE "divx" "darkflix"
    !insertmacro APP_UNASSOCIATE "ogv" "darkflix"
    !insertmacro APP_UNASSOCIATE "rm" "darkflix"
    !insertmacro APP_UNASSOCIATE "rmvb" "darkflix"
  ...
  keepUserData:
    ...
    IfSilent +2
      ExecShell "open" "https://github.com/YourOrg/darkflix-desktop"
...
Function finishpageaction
    CreateShortCut "$DESKTOP\${APP_NAME}.lnk" "$INSTDIR\darkflix.exe" "" "$INSTDIR\darkflix.exe" "" "" "" "${APP_NAME} ${PRODUCT_VERSION}"
FunctionEnd
